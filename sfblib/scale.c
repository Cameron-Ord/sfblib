...
//this is meant to be called before any rendering, immediately after loading a sprite, to apply scaling.
//Will return a heap allocated pointer to a pixel buffer. The original sprite can be used alongside it or freed and replaced.
//A seperate function that does not require any allocation and can scale on the fly is also available.
